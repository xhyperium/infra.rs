#!/usr/bin/env node
/**
 * 读取 builder 生成的私有 env 文件，并为单个子进程注入 FOUNDATIONX_*。
...
function parseArgs(argv) {
  if (argv[0] !== "--env-file" || !argv[1]) {
    fail("用法: run-foundationx-command.mjs --env-file <path> -- <command> [args...]");
  }
  if (argv[2] !== "--" || !argv[3]) {
...
function parseEnvironment(filePath) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (!/^FOUNDATIONX_[A-Z0-9_]+$/u.test(key)) {
...
function main() {
  ...
  const childEnv = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => !key.startsWith("FOUNDATIONX_")),
  );
