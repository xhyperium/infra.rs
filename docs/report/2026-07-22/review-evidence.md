...
| D3. 错误处理 | 2/5 | 仅 2 个变体；英文 Display（违中文约定）；无 `XError`/`ErrorKind` 映射；无 `source()` 链 |
...
2. **中期（P2 改进）**：`FileEvidenceAppender` 的 IO 错误区分；`RemoteEvidenceAppender` 的传输语义文档化；SSOT 战占位声明。
