...
错误分类、监控打点、关停、drain、配置可观测。

**方法：** kernel ShutdownSignal；bootstrap require_evidence；observex 最小 tracing；adapters health/pool。

**本轮结论（一句话）：** 关停信号有、组合根 async drain 与 OTEL 无；运维面不足以支撑交易平台。

**硬边界：** 本轮**不**构成 Maintainer L5 签核；**不**宣称 workspace Production Ready。

## 2. 本轮聚焦表

| Package | 层 | Go | Σ | DEFER |
|---------|------|------|------|------|
| `kernel` | L1+L4 | 有条件 GO（库语义） | 34 | archgate OOS; 组合根 drain 在 bootstrap |
| `testkit` | L1 test-support | 有条件 GO（仅测试） | 33 | integration harness DEFER |
| `decimalx` | L1 | 有条件 GO（内部） | 33 | wire 跨版本 stable; panicking 算子仍公开 |
| `canonical` | L2 subset v1–v1.3 | 有条件 GO（committed wire） | 33 | 无 schema_version envelope |
| `bootstrap` | L1 有条件 | NO-GO 交易装配 | 32 | StoreSet/adapter 未接线; async drain DEFER |
| `configx` | L1 内存合同 | 合同内 GO / 配置中心 NO | 32 | 多源/热更新/secret DEFER |
| `schedulex` | L1 registry | 登记 GO / 调度 NO | 32 | 无 timer/cron/Job 执行 |
| `evidence` | L1 append | 开发默认 GO / 合规 NO | 25 | 远程/签名 wire; 查询 API |
| `observex` | L1 + L3 Instr 入口 | 最小面 GO / OTEL NO | 32 | OTEL exporter/flush DEFER |
| `resiliencx` | 接近 L1 Internal | 有条件 GO（同步原语） | 31 | budget; 未接入 adapters |
| `transportx` | L1 有条件 I/O | 有条件 GO | 32 | TLS 矩阵/池/代理 DEFER |
| `contracts` | L3 子集 KV+Instr | 子集 GO / first-batch NO | 33 | Tx/Bus/Repo/Venue 业务 live |
| `contract-testkit` | L1 test-support | 有条件 GO（仅 dev） | 29 | Batch-2 Fake; 真后端 profile |
| `binancex` | scaffold + server_time | NO-GO 交易 | 24 | 签名/下单/WS 行情 |
| `okxx` | scaffold + server_time | NO-GO 交易 | 24 | 签名/业务协议 |
| `redisx` | L1 + L3-KV 入口 | 有条件 GO（KV） | 33 | Cluster/Sentinel; TLS 强制; resiliencx |
| `postgresx` | L1 池+Tx | 有条件 GO（SQL） | 33 | prod Repository; SSL require; resiliencx |
| `kafkax` | L1 AMO EventBus | 有条件 / EOS NO | 31 | offset commit; at-least-once; EOS |
| `natsx` | L1 Core NATS | Core GO / JetStream NO | 31 | JetStream; TLS 默认 |
| `ossx` | L1 ObjectStore | 有条件 GO | 28 | multipart; retry |
| `clickhousex` | L1 HTTP 部分 | 部分 / 批量 NO | 27 | 批量 insert; 池强度 |
| `taosx` | L1 REST 部分 | 部分 | 27 | 批量写; native; 池 |
...
- R5 视角「可运维性」下，未发现足以推翻「整体 NO-GO」的新证据。
