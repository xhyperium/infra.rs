...
反序列化拒绝、资源有界、依赖供应链、密钥与 TLS。

**方法：** deny 配置、adapter 超时/池、scaffold feature gate、live 凭据脚本 gitignore。

**本轮结论（一句话）：** 供应链与基础有界控制可用；TLS 强制与 resiliencx 接入仍是共性缺口。
...
- R6 视角「安全性」下，未发现足以推翻「整体 NO-GO」的新证据。
