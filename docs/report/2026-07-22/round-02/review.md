...
公开 API panic 风险、金额/时钟不变量、边界与错误路径。

**方法：** 对照 decimalx checked 纪律、kernel ClockDomain、configx 毒化读、adapters 错误映射。

**本轮结论（一句话）：** 核心类型正确性合同较强；exchange 业务路径几乎不存在，无从谈正确性闭合。
...
- R2 视角「正确性」下，未发现足以推翻「整体 NO-GO」的新证据。
