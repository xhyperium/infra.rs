...
| contracts | 1 | **否（L3 未过）** | 缺非 scaffold 验证入口 |
| adapters | 9 | **否（9/9）** | 内存 scaffold / mock；无真实 SDK |
...
| `redisx` | scaffold+mock | 89% | — | **不可用** | 内存 KV；TTL 仅 mock 路径 |
...
| 模块 | 实质 | 生产判定 | 最大陷阱 |
|------|------|----------|----------|
| bootstrap | typed 组合根 + 关停信号 | Internal **有条件** | release 下 `require_evidence` 可被 `build` 绕过（debug_assert） |
...
1. bootstrap release 不强制 evidence  
...
```text
应用
  ├─ bootstrap.build()          → release 可不强制 evidence
...
| ID | 项 | 平面 |
|----|-----|------|
| WS-P0-1 | 无任何 adapter **非 scaffold** 真实后端入口 | adapters + contracts L3 |
...
| contracts | **否（L3）** | 待 W4 非 scaffold 入口 |
| adapters ×9 | **否** | 待真实 SDK + live 证据 |
...
以下全部勾选前，**不得**宣称「infra.rs 模块满足生产级应用标准」：

- [ ] W0：生产 trait / DTO / 支持矩阵冻结并入库
- [ ] 至少一个 **非 scaffold** adapter 验证入口（真实后端或受控 testcontainers）有 CI 证据
- [ ] contracts first-batch L3 三条件全绿
- [ ] bootstrap `require_evidence` release fail-closed；关停 drain 合同文档化
- [ ] 配置 / 证据 / 观测 / 调度的**产品边界**写入 README 红线（禁止误用路径）
- [ ] decimal 资金路径门禁 + wire 策略 Accept 列表冻结
- [ ] 用户可见错误中文化达标（或书面豁免）
- [ ] L5 maintainer 签字 + CHANGELOG 发布说明 + DEFER 列表冻结
