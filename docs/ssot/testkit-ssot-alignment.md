...
| ID | 条款 | 状态 | 说明 |
|----|------|------|------|
| 24.1 | layer=test-support；非 L0 runtime；单 active spec；README/AGENTS 对齐 | 候选已实现 / 重审中 | `publish=false`、README/AGENTS、本仓 active SSOT |
...
| 24.0-h | Integration harness | **候选已实现 / 重审中** | `src/harness.rs` · `IntegrationHarness::{new,with_wall,step,run}` + typed terminal report/error；导出见 `src/lib.rs` |
