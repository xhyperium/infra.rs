...
```bash
# 删除感知同步（推荐）；保留 adapters/ 层级（infra 与 crates/infra 对齐）
...
| adapters 九域 | `crates/adapters/**` | **storage×7 生产默认路径** + live/bench（#188–#190）；exchange 生产默认 REST+WS（#210+#214+#215）+ public time live ignore | [adapters-ssot-alignment.md](./adapters-ssot-alignment.md) |
...
| tools/evidence | `crates/infra/evidence` | 最小面 + `FileEvidenceAppender` | [evidence-ssot-alignment.md](./evidence-ssot-alignment.md) |
| tools/goalctl·verifyctl | `tools/goalctl` · `tools/verifyctl` | **workspace member** 最小生产 CLI（#188） | [tools-ssot-alignment.md](./tools-ssot-alignment.md) |
| tools/xtask | — | **未** member | 同上 |
...
## 补充：infra 平面镜像（2026-07-21，v0.3.18；2026-07-24 已恢复 infra/ 层级）

> **已废弃**：2026-07-24 起 infra 域规格恢复 `infra/` 层级（见 SSOT.md v2.3.0），不再展平到根。以下为历史记录，保留供参考。

infra 的 8 个子域曾从 `.agents/ssot/infra/` 展平到 `.agents/ssot/` 根（v2.1.0）：

**源**: `/home/workspace/xhyper.rs/.agent/SSOT/infra/`（上游仍保留 `infra/` 层级）  
**历史目标**: `/home/workspace/infra.rs/.agents/ssot/`（v2.1.0 展平状态，已废弃）  
**当前目标**: `/home/workspace/infra.rs/.agents/ssot/infra/`（v2.3.0 恢复）
...
> 镜像 COMPLETE ≠ 本仓业务实现。**storage×7 生产默认路径**已落地（#188–#190）+ live/bench；exchange **生产默认 REST+WS**（#210+#214+#215）；live 仅 `server_time` ignore（#172 入口保留）。
> 状态见 [adapters-ssot-alignment.md](./adapters-ssot-alignment.md)。
...
## 补充：tools 本仓 SSOT（2026-07-21 · 刷新 2026-07-22）

**路径**: `.agents/ssot/tools/`（保留 `tools/` 层级）

| 子域 | 本仓状态 |
|------|----------|
| evidence | SSOT 已就位；crate 最小面 + `FileEvidenceAppender`（`evidence`） |
| goalctl | SSOT 已就位；**workspace member** `tools/goalctl`（#188） |
| xtask | SSOT 已就位；**无** crate |
| verifyctl | SSOT 已就位（本仓扩展）；**workspace member** `tools/verifyctl`（#188） |
...
### 2026-07-22 · draft storage/tools 生产落地（#188–#191）

| 增量 | 说明 |
|------|------|
| storage×7 | 默认生产客户端 + live `#[ignore]` + 有界 benches |
| goalctl / verifyctl | workspace members；自验证 schema v1 |
| live env | `scripts/live/build-foundationx-env.mjs`（ZoneCNH secrets → FOUNDATIONX_*） |
| 对齐文 | workspace / adapters / tools / gap-matrix 已刷新 |
