...
| kafkax | Producer pool + EventBus | **done** + 生产测试矩阵 offline/reliability/bench/fault (`0.3.7`) | SCRAM 成功路径/group/rebalance/native EOS/DLQ/Part2 OOS/24h 默认 soak |
...
| taosx | TimeSeries write+query | **done** + REST/NCHAR + BatchWriteReport + WS 探测 + 资源上界 + 真实 live + 十轮矩阵 (`0.3.7`) | 完整 WS SQL 会话 / HA / 自动幂等重试 / package stable |
