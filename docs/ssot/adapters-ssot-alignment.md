...
| `.agents/ssot/adapters/storage/kafka` | `crates/adapters/storage/kafka` | `kafkax` | **`0.3.7`** headers/key/stats + selfcheck + 生产矩阵；group/native EOS NO-GO；Part2 OOS |
...
| `.agents/ssot/adapters/storage/redis` | `crates/adapters/storage/redis` | `redisx` | **`0.3.12`** Standalone P0+ KV/pipeline/锁/metrics/selfcheck + live 矩阵 + data E2E soft-skip；Cluster/Sentinel/TLS live OPEN；package stable 禁止宣称 |
