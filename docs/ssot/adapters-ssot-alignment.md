...
| `.agents/ssot/adapters/storage/kafka` | `crates/adapters/storage/kafka` | `kafkax` | **`0.3.4`** AMO/ALO + TLS/CA/PLAIN + 真 secrets live + 十轮矩阵；group/native EOS NO-GO；Part2 OOS |
...
| `.agents/ssot/adapters/storage/postgres` | `crates/adapters/storage/postgres` | `postgresx` | **`0.3.9`** Pool/Tx/Repository + 远程 Require live（CA+SNI）；dev live 10/10 + remote TLS 9/9 + deadline；mTLS/package stable OPEN |
...
| `.agents/ssot/adapters/storage/taos` | `crates/adapters/storage/taos` | `taosx` | **`0.3.6`** REST/NCHAR + BatchWriteReport + 有界 metrics + Native WS live 握手；幂等重试/package stable NO-GO |
验证（本仓权威命令）：
