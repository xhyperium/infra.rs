...
| Active SSOT | storage：`.agents/ssot/adapters/storage/**`；exchange 规格已收敛至 `.agents/ssot/market_data/{binance,okx}/**`（变更须同步 dual spec、alignment 与门禁） |
...
exchange（binance/okx）规格已于 2026-07-24 收敛至 `market_data/`，不再保留 `adapters/exchange/` SSOT 副本。

```text
.agents/ssot/
├── market_data/        # exchange（binance/okx）行情/适配器规格（收敛后唯一源）
...
标准布局八项                    已齐
publish                         false（显式）
version                         storage 独立版本；见下表各 package 行（kafkax `0.4.0`）

| 镜像路径 | 本仓路径 | package | 本仓状态 |
|----------|----------|---------|----------|
| `.agents/ssot/market_data/binance` | `crates/adapters/exchange/binance` | `binancex` | **`0.3.2`** HMAC 签名 REST + 公共 WS 解析/注入；live 仅 server_time；交易 NO-GO |
| `.agents/ssot/market_data/okx` | `crates/adapters/exchange/okx` | `okxx` | **`0.3.3`** 四头签名 REST + 公共 WS 解析/注入；live 仅 server_time；交易 NO-GO |
...
| `.agents/ssot/adapters/storage/kafka` | `crates/adapters/storage/kafka` | `kafkax` | **`0.4.0`** headers/key/stats + G-STATS-01 严格 cancel/timeout 计数 + selfcheck + 生产矩阵；group/native EOS NO-GO；Part2 OOS |
| `.agents/ssot/adapters/storage/nats` | `crates/adapters/storage/nats` | `natsx` | **`0.3.5`** Core/JetStream；同客户端重启恢复 3/3，断线窗口无回放、Cluster/HA NO-GO |
| `.agents/ssot/adapters/storage/oss` | `crates/adapters/storage/oss` | `ossx` | **`0.4.1`** ObjectStore + 有界 multipart/retry/orphan 补偿；dev live PASS |
| `.agents/ssot/adapters/storage/postgres` | `crates/adapters/storage/postgres` | `postgresx` | **`0.3.13`** Pool/Tx/Repository/COPY/Migrator/mTLS/selfcheck；live + selfcheck Full；package stable OPEN |
| `.agents/ssot/adapters/storage/redis` | `crates/adapters/storage/redis` | `redisx` | **`0.3.15`** 全公开 API（KV/结构/Streams/事务/锁/selfcheck）+ live/E2E/bench；Cluster/Sentinel/TLS live 环境 OPEN；package stable 禁止宣称 |
| `.agents/ssot/adapters/storage/taos` | `crates/adapters/storage/taos` | `taosx` | **`0.3.10`** Production-default 全 API + gap-zero register；非 crates.io package-stable |
验证（本仓权威命令）：

```bash
# 双镜像（exchange 已收敛至 market_data/；storage 仍在 adapters/storage/）
for d in binance okx; do
  cmp .agents/ssot/market_data/$d/spec/spec.md \
      .agents/ssot/market_data/$d/spec/xhyper-*-complete-spec.md
done
for d in clickhouse kafka nats oss postgres redis taos; do
  cmp .agents/ssot/adapters/storage/$d/spec/spec.md \
      .agents/ssot/adapters/storage/$d/spec/xhyper-*-complete-spec.md
done
