...
标准布局八项                    已齐
publish                         false（显式）
version                         storage 独立版本；见下表各 package 行（kafkax `0.3.9`）
...
| `.agents/ssot/adapters/storage/kafka` | `crates/adapters/storage/kafka` | `kafkax` | **`0.3.9`** headers/key/stats + G-STATS-01 严格 cancel/timeout 计数 + selfcheck + 生产矩阵；group/native EOS NO-GO；Part2 OOS |
