...
- [ ] 无公开可构造非法状态导致 panic
- [ ] oracle 差分与边界矩阵在 CI 或 scheduled 有证据
- [ ] 资金路径 panicking ops 有门禁或明确 exclude 列表
- [ ] 模块 README 仍不写整体 Production Ready，直到 L5
...
- [ ] 本轮声明的 committed 清单 100% 满足 §7.3 八项
- [ ] Uncommitted 类型 rustdoc 仍显式标注
- [ ] 无「全 wire Production Ready」措辞
- [ ] `node scripts/quality-gates/check-canonical-align.mjs` 绿
...
- [ ] 首批 trait 均有语义文档 + Fake + conformance suite
- [ ] in-tree venue adapter override 门禁绿
- [ ] `public_surface` 驱动真实路径（禁止占位 `assert_eq!(15,15)` 类）
- [ ] bootstrap 仅 re-export / `Bounded*`，无静默同名冲突
...
- [ ] 首批 trait 每个都有「Fake 合同绿 + live/mock 入口绿」证据路径
- [ ] scaffold 文档不再被误读为生产实现
- [ ] DEFER-1 可 Close 或降为「非首批 trait 仍 scaffold」
...
1. W0 清单冻结且无未分类 DEFER。
2. decimalx 达 L1（W1 验收勾选）。
3. canonical 本轮 committed 子集达 L2（W2）。
4. contracts 首批 trait 达 L3（W3+W4）。
5. kernel/testkit 维持/完成 L1（+ kernel L4 矩阵声明）。
6. W5 门禁合并；**maintainer 完成 DEFER-7 签字包**。
7. 审计报告 §11 或后续 follow-up 报告更新为最终状态；**禁止**在签字前改写为整体 Production Ready。
