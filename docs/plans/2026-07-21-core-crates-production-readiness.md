...
| **P0 contracts** 仅接口形状 | **部分闭合**：`TxContext`/`begin_tx` 对象安全、`BusMessage`/`MessageAck`、**独立** `contract-testkit`（#178）、bootstrap `Bounded*` | 全 trait 深度语义、真实后端入口、VenueAdapter override 编译门禁 |
...
3. **adapters 是内存 scaffold**：`PostgresAdapter::begin_tx` 使用本地 `ScaffoldTxContext`（#178 后；仍非真实事务），不能证明事务原子性。
...
| W3-9 | 独立 `contract-testkit` 路径决策 | **Done #178**：`crates/test-support/contracts`；Fake 迁出 contracts |
...
adapters 全为内存 scaffold（例：postgres `begin_tx` → 本地 `ScaffoldTxContext`）。没有真实后端，L3 不能签字。
