...
- CI、测试和 API 纪律必须以本文件为输入；
- **infra.rs 不适用（OOS）**：本仓不维护 `.architecture/**`，不运行/不移植 archgate；本仓机控 = 结构扫描 / unit tests / CI（coverage, loom, miri, public-api）；
...
**设计意图**（错误治理基线）：应维护 `XError::internal` 使用点数量、owner、创建/到期日期与替代分类计划；基线只允许下降，不允许无审批增加。

> **infra.rs 不适用（OOS）**：历史 monorepo 由 `archgate` 机控本项；本仓**不**以 archgate 为验收条件。本仓可用结构扫描 / code review / unit tests 约束 internal 棘轮，**不发明新工具**。
...
**设计意图**：`from_clock_elapsed` 调用位置应仅出现在：
...
历史 monorepo 由 **KERNEL-TIME-004**（`FROM_CLOCK_ELAPSED_ALLOW`）机控。

> **infra.rs 不适用（OOS）**：本仓不运行 archgate；调用边界靠结构扫描 / review / unit tests，**不**作为 archgate 验收门禁。
...
# 本仓机控轨（按 CI 实际接线；不强制下列 monorepo 专用工具）：
...
> **infra.rs 不适用（OOS）**：下列 KERNEL-* 规则描述 **设计意图与纪律**。历史 monorepo 由 `archgate` 机控；**本仓明确不移植 archgate**，也不维护 `.architecture/**`。本仓机控 = 结构扫描 / unit tests / CI（coverage, loom, miri, public-api）。**勿发明新工具**。
...
KERNEL-API-001:
  公开 API 必须与冻结清单一致（本仓可用 cargo-public-api / CI，非 archgate）。

KERNEL-API-002:
  新增公开项未关联 Approved RFC → 拒绝（纪律；非 archgate 门禁）。

KERNEL-PUBLISH-001:
  Cargo 与本 Spec 的 publish 值应一致（历史含 architecture registry 三方一致；本仓不维护 registry 文件）。

KERNEL-TIME-001:
  kernel 之外生产代码出现 SystemTime::now → 禁止（结构扫描 / review）。

KERNEL-TIME-002:
  kernel 之外生产代码出现 Instant::now → 禁止（结构扫描 / review）。

KERNEL-TIME-003:
  Timestamp::from_unix_nanos 调用位置不在允许清单 → 禁止/警告。
...
KERNEL-SERDE-001:
  kernel 公开类型实现 Serialize/Deserialize → 禁止。

KERNEL-ASYNC-001:
  kernel 依赖或公开签名出现 tokio/async-std 类型 → 禁止。

KERNEL-UNSAFE-001:
  unsafe 代码数量必须为 0。

KERNEL-LIFECYCLE-001:
  ShutdownSignal 实现未通过 loom test → 不得宣称关停正确性闭合。
```

## 12.3 Public API 快照

**设计意图**：公开 API 应有可 diff 的冻结清单，来源优先级：
...
> **infra.rs 不适用（OOS）**：本仓**不**维护 `.architecture/api/kernel-public-api.txt`。历史 monorepo 路径仅作记录；本仓以 `cargo public-api` / CI / 审查为准。
...
├── coverage.json
├── mutants.json
├── public-api.diff          # 可选；本仓 public-api / CI 输出
├── downstream-impact.md
└── verdict.md
# 历史 monorepo 另可含 archgate.json → **infra.rs 不适用（OOS）**，不要求生成
```
...
- public API 从当前源码重新生成并完成 diff（本仓：public-api / CI / 审查）；
- KERNEL-* 设计意图满足（含 API-002、TIME-004、PUBLISH-001 语义）；**archgate 机控 infra.rs 不适用（OOS）**；
- 本仓质量门禁通过：结构扫描 / unit tests / CI（coverage, loom, miri, public-api）；历史 monorepo 的 lint-deps 等工具按仓库实际接线；
- Internal 所有等价构造路径均受棘轮约束（结构扫描 / review，非 archgate）；
- 下游迁移完成；
