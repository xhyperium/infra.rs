...
> | archgate | **16/16** KERNEL-*（含 API-001 源码生成比较、API-002、PUBLISH-001、TIME-004） |
...
**SPEC-KERNEL-002** 主路径已 **merge 进 main**（PR #235，merge `e7bda98e`，2026-07-14），已 tag `kernel-v0.1.1`，已 publish crates.io `xhyper-kernel` 0.1.1。落地事实包括：archgate KERNEL-* **16/16 机控 PASS**（固定版本 `cargo-public-api` 从当前源码生成并与 snapshot/baseline 比较；删除/未授权 addition fail closed；Cargo/workspace/Spec publish 三方一致；TIME/ERR/SERDE/ASYNC/UNSAFE/LIFECYCLE 等）、API 快照文件存在、registry `stable`、version `0.1.1`、residual OPEN=0（branch/miri/mutants **ad-hoc 实测 PASS**；CI 门禁化见 P1）。
...
| 门禁 | KERNEL-* **16/16**；API 从当前源码 regenerate/diff；完整 Cargo metadata；publish 三方一致 | `tools/archgate/src/kernel_rules.rs` |
...
6. **机器强制（诚实边界）**：archgate KERNEL-* **16/16（含 API-002 + TIME-004）** + lint-deps R1–R6；**不**把窄扫描写成全覆盖。
...
5. `XError::internal` 业务侧调用点 ≤ `XERROR_INTERNAL_BASELINE`（当前 **8**，**KERNEL-ERR-001 机控**）。
6. **规范（应）**：反应决策基于 `kind()`，禁止用 `Display`/任意字符串匹配决定重试或分类。  
   **机控（是）**：KERNEL-ERR-002 仅扫描 **legacy 窄模式**——同行内同时出现 `ErrorKind::`/`XError::` 与 `"not_found"` / `"not found"` / `"other"` / `contains("not_found")` 等 residual 字符串；**不**拦截 `err.to_string().contains("transient")`、自定义 Display 匹配等。完整禁令依赖 code review / 未来规则加严。
...
5. **墙钟/单调 now 的 archgate 边界（按实装，非「仅 SystemClock 函数体」）**：
   - **KERNEL-TIME-001**：在 **非** `crates/kernel/` 的生产 `.rs` 中禁止 `SystemTime::now` **与** `Utc::now`；**排除整个 `tools/`** 前缀（tools 不扫）。
   - **KERNEL-TIME-002**：`Instant::now` 仅允许路径前缀 `crates/kernel/`（**整个 kernel 树**，不限于 `SystemClock` 函数体字面）；`tools/` 不扫。
   - **规范意图**仍是：业务生产路径应只经 `Clock`/`SystemClock` 取时；机控粒度粗于「仅 SystemClock 实现行」。
6. **构造入口分轨（勿混）**：
   - **KERNEL-TIME-003**：**只**扫描 `from_unix_nanos` 调用点 allowlist（`FROM_UNIX_NANOS_ALLOW`，kernel_rules.rs:145-154，共 7 项）：  
     ...
     （后几者为同文件 `#[cfg(test)]` FixedClock 字面墙钟 / 持久化还原墙钟）。`tools/` 不扫。
   - **KERNEL-TIME-004**：扫描 `from_clock_elapsed` 调用点 allowlist（`FROM_CLOCK_ELAPSED_ALLOW`，kernel_rules.rs:156-320）。**已实现**（机控 PASS）；调用位置限制已机控，不再仅靠 `#[doc(hidden)]` + review。
...
机器快照：`.architecture/api/kernel-public-api.txt`（由 `cargo-public-api` 规范化生成后入库）。  
门禁：`KERNEL-API-001` 以固定版本从当前 `xhyper-kernel` 源码 regenerate，与 candidate snapshot 比较，并拒绝 Component/serde 面；工具缺失/版本不符 fail closed。
...
1. 不得新增 `XError::internal` 调用点使总数 > baseline 8（须审批降其他点或改分类）——**KERNEL-ERR-001 机控**。
2. **规范**：反应断言用 `kind()`，禁止 match `Display` 字符串。**机控**：仅 ERR-002 legacy `not_found`/`other` 窄扫描；其余靠 review。
3. 自定义 `Clock` 必须实现 `monotonic`。
4. 生产路径避免直调 `SystemTime::now` / `Utc::now` / `Instant::now`（archgate TIME-001/002，边界见上；`tools/` 豁免）。
5. 关停正确性不以 sleep 证明；composition 须显式持有 `ShutdownGuard` 并在 OS signal 路径 `trigger`。
6. 公开 API 变更 = 全 workspace 影响；破坏性须 RFC + 迁移批次 + Evidence + 更新 `.architecture/api/kernel-public-api.txt`。KERNEL-API-001 会从当前源码 regenerate 规范化 public API 并与 candidate snapshot 比较；KERNEL-API-002 相对冻结 baseline 拒绝 removal/signature change，addition 须 Approved RFC。
...
flowchart LR
  ...
  SRC --> SNAP[.architecture/api/kernel-public-api.txt]
  SRC --> AG[archgate KERNEL-*]
  AG --> CI[CI]
  SNAP --> AG
  SRC --> LD[xtask lint-deps R1-R6]
  LD --> CI
  SRC --> LOOM[CI kernel-loom]
  LOOM --> CI
```

### 命名规则一览（`tools/archgate/src/kernel_rules.rs` 实装为准）

| Rule ID | 机控实际做什么 | 状态 |
|---------|----------------|------|
| KERNEL-DEP-001 | `[dependencies]` 中 workspace 包名 = 0 | 机控 PASS |
| KERNEL-DEP-002 | 外部生产依赖仅 `thiserror` | 机控 PASS |
| KERNEL-FEATURE-001 | features 仅 `default` 且空，或无 features 节 | 机控 PASS |
| KERNEL-API-001 | 固定版本 `cargo-public-api`（当前 pin `0.52.0`）从当前 `xhyper-kernel` 源码生成规范化 public API；与 candidate snapshot 比较，stale/不一致 fail closed；禁 Component/serde 面；工具缺失或版本不符 fail closed（无 snapshot 回退） | 机控 PASS |
| KERNEL-API-002 | 相对冻结 baseline（行数 + FNV 指纹）：removal/signature change 一律拒绝；addition 须在 `kernel-api-rfc.toml` 逐行登记 **Approved** RFC；源码 API 不可用时不得回退 committed snapshot | 机控 PASS |
| KERNEL-PUBLISH-001 | `crates/kernel` Cargo.toml、workspace registry unit、kernel Spec 三方 `publish` 布尔一致 | 机控 PASS |
| KERNEL-TIME-001 | 非 kernel 且非 `tools/`：禁 `SystemTime::now` **与** `Utc::now` | 机控 PASS |
| KERNEL-TIME-002 | 非 `tools/`：`Instant::now` 仅 `crates/kernel/` 前缀 | 机控 PASS |
| KERNEL-TIME-003 | 非 `tools/`：`from_unix_nanos` 仅 `FROM_UNIX_NANOS_ALLOW` 路径（见 clock 不变量 6） | 机控 PASS |
| KERNEL-TIME-004 | `from_clock_elapsed` 调用位置须在 `FROM_CLOCK_ELAPSED_ALLOW`（kernel_rules.rs:316-320） | 机控 PASS |
| KERNEL-ERR-001 | `crates/**` 消费侧 `XError::internal` 计数 ≤ **8**（排除 kernel） | 机控 PASS |
| KERNEL-ERR-002 | **窄** residual：同行 `ErrorKind::`/`XError::` + `not_found`/`not found`/`other` 等字面 | 机控 PASS（非全 Display 禁令） |
| KERNEL-SERDE-001 | kernel src 无 serde 相关 token | 机控 PASS |
| KERNEL-ASYNC-001 | kernel src 无 tokio/async-std token | 机控 PASS |
| KERNEL-UNSAFE-001 | kernel unsafe **用法** = 0（排除 forbid 属性行） | 机控 PASS |
| KERNEL-LIFECYCLE-001 | **仅资产**：`tests/lifecycle_concurrency_loom.rs` 存在且含 `loom::model` / `ShutdownSignal` / `cfg(loom)`。**不**验证 CI 配置或执行结果 | 机控 = 资产；**执行** = `.github/workflows/ci.yml` job `kernel-loom`（`RUSTFLAGS=--cfg loom` + `cargo test -p kernel --test lifecycle_concurrency_loom`） |

**合计**：**已实现命名规则机控 16/16**（DEP-001/002、FEATURE-001、API-001/002、PUBLISH-001、TIME-001…004、ERR-001/002、SERDE/ASYNC/UNSAFE-001、LIFECYCLE-001）。  
**不得**解读为「SPEC §12.2 全量机器强制完成」——ERR-002 仍为窄 residual 扫描、LIFECYCLE-001 为 loom 资产存在性（动态执行靠 CI `kernel-loom`），勿高估。API-001 **已**绑定源码 regenerate，不再是“仅文件存在”。
...
8. Evidence 包 + CHANGELOG [Unreleased]
9. 破坏性：版本策略 + 回滚方案（baseline 重冻须改 archgate 指纹常量并走审查）
```
...
gantt
  ...
  G2 loom + archgate + API 快照            :done, 3, 4
  section Release
...
- [ ] 未声称 from_clock_elapsed / 全 Display 禁令已被 archgate 全覆盖
---
...
| Model | `lifecycle_concurrency_loom.rs` | lost wake-up 窗口（archgate 只验**资产**） |
| CI 执行 | `.github/workflows/ci.yml` → `kernel-loom` | `RUSTFLAGS=--cfg loom` 下跑 loom 测试（**非** archgate 内执行） |
...
# loom（与 CI kernel-loom 对齐；RUSTFLAGS='--cfg loom'）
...
| 门禁 | archgate JSON；CI kernel-loom；internal baseline 棘轮 |
...
| internal 基线上涨 | archgate fail；须审批与迁移计划 |
...
| KD-15 | archgate KERNEL-* **16 条已实现**，含源码 API regenerate/diff、完整 Cargo metadata 依赖检查与 publish 三方一致性 | 机器强制优于口头纪律 |
...
| API 快照 | `.architecture/api/kernel-public-api.txt`（人工维护；API-001 不自动 diff） |
