...
| **Code** | **`crates/contracts`** | 实现不在 `.agent/SSOT/` |
...
```bash
cmp .agent/SSOT/contracts/spec/spec.md \
    .agent/SSOT/contracts/spec/xhyper-contracts-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/contracts/README.md
test -f .agent/SSOT/contracts/spec/spec.md
```
