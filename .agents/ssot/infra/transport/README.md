# infra/transport — Goal 管线契约

> 实现 / 代码唯一位置：`crates/transport`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/transportx-complete-spec.md](spec/transportx-complete-spec.md)  
...
| **Code** | **`crates/transport`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/transportx-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/infra/transport/spec/spec.md \
    .agent/SSOT/infra/transport/spec/transportx-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/infra/transport/README.md
test -f .agent/SSOT/infra/transport/spec/spec.md
```
