...
- Candidate：[SPEC-INFRA-TRANSPORTX-002](../../../../draft/xhyper-transportx-complete-spec.md)（Draft，非权威，不覆盖本文）
...
```bash
cargo test -p xhyper-transportx
cargo check -p xhyper-transportx --all-targets
cargo clippy -p xhyper-transportx --all-targets -- -D warnings
cargo test -p xhyper-binance
cargo test -p xhyper-okx
cargo xtl lint-deps
cargo fmt -- --check
```
