# infra/schedulex — Goal 管线契约

> 实现 / 代码唯一位置：`crates/infra/schedulex`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-schedulex-complete-spec.md](spec/xhyper-schedulex-complete-spec.md)  
...
| **Code** | **`crates/infra/schedulex`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-schedulex-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/infra/schedulex/spec/spec.md \
    .agent/SSOT/infra/schedulex/spec/xhyper-schedulex-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/infra/schedulex/README.md
test -f .agent/SSOT/infra/schedulex/spec/spec.md
```
