...
> 实现 / 代码唯一位置：`crates/infra/bootstrap`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-bootstrap-complete-spec.md](spec/xhyper-bootstrap-complete-spec.md)  
...
> **状态**：`0.3.3` 进程内合同已加固；发布门禁 BLOCKED，战役未闭合
...
| Design | [design/design.md](design/design.md) | 当前 typed composition / shutdown 设计 |
...
| **Code** | **`crates/infra/bootstrap`** | 实现不在 `.agents/ssot/` |
...
| Release | [release/release.md](release/release.md) | `0.3.3` 候选；BLOCKED |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-bootstrap-complete-spec.md` 须 `cmp` 同构。
...
test -f .agents/ssot/infra/bootstrap/README.md
test -f .agents/ssot/infra/bootstrap/spec/spec.md
```
