...
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/testkitx-complete-spec.md](spec/testkitx-complete-spec.md)  
...
| **Code** | **`crates/testkit`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/testkitx-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/infra/testkitx/spec/spec.md \
    .agent/SSOT/infra/testkitx/spec/testkitx-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/infra/testkitx/README.md
test -f .agent/SSOT/infra/testkitx/spec/spec.md
```
