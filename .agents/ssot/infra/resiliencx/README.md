# infra/resiliencx — Goal 管线契约

> 实现 / 代码唯一位置：`crates/resiliencx`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/resiliencx-complete-spec.md](spec/resiliencx-complete-spec.md)  
...
| **Code** | **`crates/resiliencx`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/resiliencx-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/infra/resiliencx/spec/spec.md \
    .agent/SSOT/infra/resiliencx/spec/resiliencx-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/infra/resiliencx/README.md
test -f .agent/SSOT/infra/resiliencx/spec/spec.md
```
