...
| 名称 | 路径 | 说明 |
|------|------|------|
| `xhyper-archgate` | `tools/archgate` | 架构门禁工具 — **OOS：infra.rs 不引入**（非本仓 KEEP） |
...
| active spec | `.agents/ssot/gate/gate-spec.md` | T-DEL-006 Superseded |
...
|------|------|
| `.agent/gates/` | **KEEP** |
| `tools/archgate` | **OOS（infra.rs 不引入）** — 非 KEEP；≠ runtime gate crate |
...
Do not touch as "consumers":
  VenueSafetyGate, .agent/gates, risk gate wording
  # archgate: monorepo 架构工具，≠ VenueSafetyGate / runtime gate；infra.rs 不引入（OOS）
```
