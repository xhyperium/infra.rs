# adapters/storage/taos — Goal 管线契约
...
| **Code** | **`crates/adapters/storage/taos`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-taosx-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/adapters/storage/taos/spec/spec.md \
    .agent/SSOT/adapters/storage/taos/spec/xhyper-taosx-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/adapters/storage/taos/README.md
test -f .agent/SSOT/adapters/storage/taos/spec/spec.md
```
