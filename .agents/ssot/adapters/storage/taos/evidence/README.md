# adapters/storage/taos — Evidence
...
| 类型 | 位置 |
|------|------|
| 单元/集成（离线） | `cargo test -p taosx --all-targets` 日志 |
| live | `crates/adapters/storage/taos/tests/live_smoke.rs` 等 |
| bench | `crates/adapters/storage/taos/benches/hot_path.rs` |
