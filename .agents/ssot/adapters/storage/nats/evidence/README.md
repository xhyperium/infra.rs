# adapters/storage/nats — Evidence

> 模块战役证据落盘处（≠ `crates/evidence` 生产库）。

## 本仓证据索引

| 类型 | 位置 |
|------|------|
| 单元/集成（离线） | `cargo test -p natsx --all-targets` 日志 |
| live | `crates/adapters/storage/nats/tests/live_event_bus.rs` 等 |
| bench | `crates/adapters/storage/nats/benches/hot_path.rs` |
| landing | [../plan/infra-rs-landing.md](../plan/infra-rs-landing.md) |
| draft | [../plan/infra-rs-draft-spec-goal.md](../plan/infra-rs-draft-spec-goal.md) |
| 对齐 | `docs/ssot/natsx-ssot-alignment.md` |

有新的验证输出时按 `YYYY-MM-DD/` 建日目录归档。
