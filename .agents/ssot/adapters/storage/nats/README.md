# adapters/storage/nats — Goal 管线契约

> 实现 / 代码唯一位置：`crates/adapters/storage/nats`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-natsx-complete-spec.md](spec/xhyper-natsx-complete-spec.md)  
...
- **状态**：Core AMO 与 JetStream durable pull 有 broker 证据；Cluster/HA/自动 DLQ 未承诺；package stable **未宣称**
...
| **Code** | **`crates/adapters/storage/nats`** | 实现不在 `.agents/ssot/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-natsx-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agents/ssot/adapters/storage/nats/spec/spec.md \
    .agents/ssot/adapters/storage/nats/spec/xhyper-natsx-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agents/ssot/adapters/storage/nats/README.md
test -f .agents/ssot/adapters/storage/nats/spec/spec.md
```
