...
| ID | 条款 | 状态 | 证据 |
|----|------|------|------|
| S-1 | workspace member `postgresx` | PASS | Cargo.toml `0.3.9` |
...
| S-14 | acquire_with | PASS | `PostgresPool::acquire_with` + live |
| S-15 | 有界 COPY IN/OUT | PASS | `copy_in_bytes`/`copy_out_bytes` + live |
