# adapters/storage/kafka — Goal 管线契约

> 实现 / 代码唯一位置：`crates/adapters/storage/kafka`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-kafkax-complete-spec.md](spec/xhyper-kafkax-complete-spec.md)  
...
- **状态**：AMO 与单 owner 应用 ALO 有 broker 证据；TLS/group/rebalance/native EOS **NO-GO**；package stable **未宣称**
...
| **Code** | **`crates/adapters/storage/kafka`** | 实现不在 `.agents/ssot/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-kafkax-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agents/ssot/adapters/storage/kafka/spec/spec.md \
    .agents/ssot/adapters/storage/kafka/spec/xhyper-kafkax-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agents/ssot/adapters/storage/kafka/README.md
test -f .agents/ssot/adapters/storage/kafka/spec/spec.md
```
