...
| **Code** | **`crates/adapters/storage/redis`** | 实现不在 `.agent/SSOT/` |
...
```bash
cmp .agent/SSOT/adapters/storage/redis/spec/spec.md \
    .agent/SSOT/adapters/storage/redis/spec/xhyper-redisx-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/adapters/storage/redis/README.md
test -f .agent/SSOT/adapters/storage/redis/spec/spec.md
```
