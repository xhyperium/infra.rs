# adapters/exchange/binance — Goal 管线契约

> 实现 / 代码唯一位置：`crates/adapters/exchange/binance`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-binance-complete-spec.md](spec/xhyper-binance-complete-spec.md)  
...
| **Code** | **`crates/adapters/exchange/binance`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-binance-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/adapters/exchange/binance/spec/spec.md \
    .agent/SSOT/adapters/exchange/binance/spec/xhyper-binance-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/adapters/exchange/binance/README.md
test -f .agent/SSOT/adapters/exchange/binance/spec/spec.md
```
