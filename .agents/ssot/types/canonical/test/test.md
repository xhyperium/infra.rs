...
| 更新 | 2026-07-23 |
| 本轮证据 | `c4604ce` 机器门禁 PASS；`fff07ea` 独立终审 GO。R1/R2 仅作历史回归输入，不继承 GO |

## 必须覆盖

| 面 | 断言 |
|---|---|
| Wire inventory | v1/v1.1/v1.2/v1.3 共 12 个类型；公开 integration table 对 12/12 `committed_wire_version` 返回精确 `WireVersion` |
...
| Envelope | shape round-trip、缺/重复/未知字段及负版本拒绝、显式版本验证成功/失败；反序列化不自动路由 |
