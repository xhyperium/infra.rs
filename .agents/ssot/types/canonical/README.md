...
> 实现 / 代码唯一位置：`crates/types/canonical`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-canonical-complete-spec.md](spec/xhyper-canonical-complete-spec.md)  
...
| **Code** | **`crates/types/canonical`** | 实现不在 `.agent/SSOT/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-canonical-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agent/SSOT/types/canonical/spec/spec.md \
    .agent/SSOT/types/canonical/spec/xhyper-canonical-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agent/SSOT/types/canonical/README.md
test -f .agent/SSOT/types/canonical/spec/spec.md
```
