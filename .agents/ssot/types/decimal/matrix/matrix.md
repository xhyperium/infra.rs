...
| Display → FromStr 全表示往返 | §5 | `Display`、`FromStr` | `boundary_matrix.rs` + property 测试 | 实现与 focused 测试 PASS；全量复验待运行 |
| DecimalError → XError source | §6 | `From<DecimalError> for XError` | `boundary_matrix.rs`（含类型 downcast） | 实现与 focused 测试 PASS；全量复验待运行 |
...
| 完成状态 | 阻断修复 focused PASS；新固定候选待全量门禁与终审 | Goal Achieved / package stable |
