...
十进制文本，拒绝 NaN / Inf、非法字符、mantissa 溢出和 `scale > 18`。
...
- [ ] 所有可表示 `Decimal` 的 `Display → FromStr` 数值往返通过边界与 property 测试。
- [ ] `DecimalError → XError` 保留可访问的 source chain。
- [x] panicking 运算符由 default-off feature 隔离；生产路径只用 `checked_*`。
- [x] serde v1 限定为内部 Rust JSON shape；跨语言 `i128` 精确承载保持 residual。
- [ ] 实现变更完成后，由执行/验证任务运行 [test](../test/test.md) 与 [gate](../gate/gate.md)
  所列门禁并保存证据。

本文的 unchecked 项是本轮实现与验证合同，不代表 Goal Achieved / Spec Approved。独立 reviewer
仍须依据完整 diff 和门禁证据给出裁决。
