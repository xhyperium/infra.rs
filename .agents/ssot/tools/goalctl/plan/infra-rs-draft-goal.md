# infra.rs draft goalctl-goal 入库（只读快照）

> **来源**：`.cargo/draft/goalctl-goal.md`
> **入库原因**：draft 合同进入本仓 SSOT；权威实现以 workspace members + 测试为准
> **本仓落地**：`tools/goalctl` member（#188）；见 docs/ssot/tools-ssot-alignment.md
...
`goalctl` 必须成为 infra.rs 唯一的 Goal Contract 编译器，把人类或 Agent 的目标意图转换成确定、可验证、可追踪、可哈希的机器契约。
