...
crates/infra/evidence                    Evidence Core
...
test ! -d .config/goal
cargo fmt --all --check
cargo clippy -p goalctl --all-targets -- -D warnings
cargo test -p goalctl
cargo run -p goalctl -- doctor --json
cargo run -p goalctl -- index --module kernel
cargo run -p goalctl -- index --module goalctl
...
```bash
cargo test -p goalctl
cargo run -p goalctl -- resolve --module kernel
cargo run -p goalctl -- resolve --module goalctl
cargo run -p goalctl -- artifact index --module kernel --mode mixed
cargo run -p goalctl -- artifact index --module goalctl --mode mixed
cargo run -p xtask -- lint-deps
# monorepo-only（infra.rs 不移植 archgate，不作为本仓 CI 硬门禁）:
# cargo run -p archgate -- --json
cargo run -p xtask -- crate-standard --check
