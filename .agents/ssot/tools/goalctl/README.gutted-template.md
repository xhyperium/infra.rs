# tools/goalctl — Goal 管线契约

> 实现 / 代码唯一位置：`tools/goalctl`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-goalctl-complete-spec.md](spec/xhyper-goalctl-complete-spec.md)  
...
| **Code** | **`tools/goalctl`** | 实现不在 `.agents/ssot/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-goalctl-complete-spec.md` 须 `cmp` 同构。
...
test -f .agents/ssot/tools/goalctl/spec/spec.md
```
