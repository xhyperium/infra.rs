<!-- infra.rs tools SSOT -->
> **本仓 SSOT**：`.agents/ssot/tools/xtask/`  
> **无** `tools/xtask` / `infra-xtask` workspace member（`cargo xtask` alias 可忽略）。  
> 文档 COMPLETE **≠** 本仓 ship。对齐：[docs/ssot/tools-ssot-alignment.md](../../../../docs/ssot/tools-ssot-alignment.md)

# tools/xtask — Goal 管线契约

> 实现 / 代码唯一位置：`tools/xtask`  
> **当前 SSOT Spec**：[spec/spec.md](spec/spec.md) ≡ [spec/xhyper-xtask-complete-spec.md](spec/xhyper-xtask-complete-spec.md)  
...
| **Code** | **`tools/xtask`** | 实现不在 `.agents/ssot/` |
...
4. 双镜像：`spec/spec.md` 与 `spec/xhyper-xtask-complete-spec.md` 须 `cmp` 同构。

## 验证

```bash
cmp .agents/ssot/tools/xtask/spec/spec.md \
    .agents/ssot/tools/xtask/spec/xhyper-xtask-complete-spec.md
# 结构：README + 11 层目录 + evidence/ 横切
test -f .agents/ssot/tools/xtask/README.md
test -f .agents/ssot/tools/xtask/spec/spec.md
```
