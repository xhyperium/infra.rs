...
| **P1** Core V1 | `crates/infra/evidence` + V1 + golden | **W1**：`T-CORE-*` | **对齐** |
...
| 目标 | `crates/infra/evidence`，`name=evidence`（plan §5） |
...
**false_pass 风险**：一旦有人同时加 `crates/infra/evidence` 为 member 且不改旧包名 → **整仓构建直接炸**，与「短暂共存」叙述矛盾。
...
| **ADR-012**（Accepted） | runtime → **`crates/infra/auditx`**；CI 报告**留** `tools/evidence`；与 002 的 **`crates/infra/evidence` + 删除 tools/evidence`** 正面对撞 | **完全未出现**于 plan / tasks / approval |
...
人审包缺 **A-ADR-012** 类闸门 → 执行期会「按 002 建 crates/infra/evidence」同时「ADR-012 仍要求 auditx」，双事实源。
...
| DEF-001…018 | 仅在 gap-matrix + `.worktrees/evidence-todo.md`（**gitignore**） |
...
| 同时 member `crates/infra/evidence` + `tools/evidence` 同名 | 构建红；「共存」叙事破产 |
| `T-CUT-001` 仅 DOM-001/GATE-001 | 删包后 fuzz/测试/文档仍引用旧 API |
| 忽略 ADR-012 | 合并后治理回滚或第二套 auditx 分叉 |
| residual 只写在 `.worktrees/` | fresh clone 看起来「无 OPEN DEF」 |
...
- Spec §31：`/home/workspace/.agents/ssot/tools/evidence/xhyper-evidence-complete-spec.md`（# 31）
