...
| Target package | `evidence` @ `crates/infra/evidence` · **0.1.1**（Stable 后） |
...
| 路径 | `tools/evidence` | `crates/infra/evidence` + `crates/adapters/evidence/*` + `tools/evidence-cli` |
...
| §3 | 目录结构 | ABSENT | ABSENT | ABSENT | ABSENT | W1/W4/W5 | 无 crates/infra/evidence；无 adapters/evidence；无 evidence-cli |
...
| workspace member | `Cargo.toml` | `tools/evidence` | 改为 `crates/infra/evidence` | W1 |
...
| DEF-019 | ADR-012 auditx 路径 vs 002 crates/infra/evidence | ADR-012 / plan §1.3 | GOVERNANCE |
...
| 系统 | 用途 | 路径 | 不得 |
|------|------|------|------|
| **crate `evidence`（本战役）** | 运行时业务审计链 | crates/infra/evidence + adapters | 用工作包 CI 记录冒充业务链 |
