...
| 当前 Gate | **`2e6d77d` 机器门禁 PASS；独立重审 PENDING；旧 GO 不继承** |
...
| panic 隔离 | `IntegrationHarness` | catch unwind → typed outcome/error；终态观测失败优先且保留上下文 | HAR-04、11、13 | 非通用进程隔离 |
| 生命周期 | `IntegrationHarness` | 消费型 builder；重跑/运行后追加不能编译 | HAR-02、06…08 | 不返回缓存成功、不静默丢 step |
| terminal result | `HarnessReport` / `HarnessRunError` | 成功持 clock+records；错误内部仅三种失败终态；失败保留 report+source；诊断不展开 source 对象 | HAR-03…05、10…15 | typed、可追溯 |
...
| §4 runner | HAR-01…15 | G-03、G-04、G-05、G-07 |
...
- typed runner、`thiserror` 错误、fail-closed/并发/公开方法测试和 PATCH bump：IMPLEMENTED；前一
  `c27b7ce` 与 `55b8997` 裁决均已失效；reviewer blocker 修复候选 `2e6d77d` 机器门禁 PASS，独立重审前保持 REVIEW PENDING。
