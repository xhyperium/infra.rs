...
| 版本类型   | 载体                         | 格式              | 示例     | 更新时机                          |
| ---------- | ---------------------------- | ----------------- | -------- | --------------------------------- |
| 项目版本   | 根 `Cargo.toml` `[workspace.package]` / `CHANGELOG.md` | SemVer | `0.3.0` | 工作区级 release / 编排变更       |
| 宪章版本   | `docs/constitution/08-amendments.md` | vX.Y.Z   | `v1.7.0` | 每次宪章修订                      |
...
| crate 间版本 | 互不绑定；`kernel 0.3.2` 与 `configx 0.1.5` 可共存 |
...
| 载体 | 版本 | 备注 |
|------|------|------|
| 根 `Cargo.toml` `[workspace.package]`（项目） | `0.3.0` | 编排版本，非 crate 源 |
| `CHANGELOG.md`（最新） | `0.3.0` | 项目级 |
| 宪章（`docs/constitution/`） | 见 §8.3 | 独立 |
| `crates/*` | **各 package 独立** | 禁止 workspace 继承 version |
| `tools/goalctl` | `0.2.0` | 独立 |
| `tools/verifyctl` | `0.1.0` | 独立 |
