...
- 本仓 members：`kernel` / `testkit` / `contract-testkit` / `configx` / `schedulex` / `bootstrap` / `evidence` / `observex` / `resiliencx` / `transport` / `types/*` / `contracts` / `adapters/**` / `goalctl` / `verifyctl`（storage×7 生产默认路径 #188–#190；exchange scaffold；**无** `infra-core`；gate/xtask 等未落地）
