...
`0.1.0`（见 `Cargo.toml`）。实现合同：`.agents/ssot/transport/spec/spec.md`。

## 生产误用红线

| 禁止 | 原因 |
|------|------|
| 宣称 TLS/池/平台矩阵完成 | M3 DEFER |
| 依赖本 crate 默认绝对 deadline / body 上限 | 当前合同未强制；须自管 |
...
| WS 单帧 | 4 MiB | `max_frame_bytes = 0` |
| Debug | header 脱敏 + body 仅长度 | — |
