...
`0.1.3`（见 `Cargo.toml`）。实现合同：`.agents/ssot/transport/spec/spec.md`。
...
| Debug | header/body + URL userinfo/全部 query value 脱敏 | — |

HTTP 响应按 chunk 累计，首次越界立即中止；`Retry-After` 支持 delay-seconds 与 HTTP-date。
Pool 新代码应使用 `try_new` + `checkout_lease_with`；旧手动 checkout/return 仅为兼容保留。
