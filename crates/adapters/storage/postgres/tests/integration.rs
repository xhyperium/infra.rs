...
use contracts::{Repository, TxRunner, run_tx_lifecycle};
use kernel::{ErrorKind, XError};
use postgresx::{
    PostgresConfig, PostgresConfigBuilder, PostgresPool, SslMode,
    PgConnection, PgTransaction, PgRecord, PgRepository, PgTxRunner,
    TxStatus, Migration, Migrator, MigrationStatus,
    PoolStats, PgRetryConfig, with_retry_sync,
    DEFAULT_PORT, DEFAULT_MAX_POOL_SIZE,
};
use std::sync::Arc;
use std::time::Duration;

fn config() -> PostgresConfig { PostgresConfig::from_env().expect("need PG env vars") }
...
async fn pool_acquire_and_execute() {
    ...
    pool.execute(&format!("CREATE TEMP TABLE {t} (id INT PRIMARY KEY, val TEXT)"), &[]).await.expect("create");
    let n = pool.execute(&format!("INSERT INTO {t} VALUES ($1,$2),($3,$4)"), &[&1i32, &"a", &2i32, &"b"]).await.expect("insert");
    assert_eq!(n, 2);
    drop(pool);
}
...
async fn pool_query_one_and_opt() {
    let pool = PostgresPool::connect(&config()).await.expect("connect");
    let row = pool.query_one("SELECT $1::int AS n, $2::text AS t", &[&42i32, &"ok"]).await.expect("query_one");
...
async fn pool_query_multi_and_copy() {
    ...
    pool.execute(&format!("CREATE TEMP TABLE {t} (id INT, data BYTEA)"), &[]).await.expect("create");
    pool.copy_in_bytes(&format!("COPY {t} FROM STDIN CSV"), b"1,hello\n2,world\n").await.expect("copy_in");
    let out = pool.copy_out_bytes(&format!("COPY {t} TO STDOUT CSV"), 1024).await.expect("copy_out");
    assert!(!out.is_empty());
    drop(pool);
}
...
async fn tx_begin_commit_rollback() {
    ...
    pool.execute(&format!("CREATE TEMP TABLE {t} (id INT PRIMARY KEY)"), &[]).await.expect("create");
...
async fn tx_with_transaction_commit_and_rollback() {
    ...
    let result = pool.with_transaction(async |tx| {
        tx.execute(&format!("INSERT INTO {t} VALUES ($1)", t = t), &[&1i32]).await?;
        Ok::<_, XError>(42u32)
    }).await.expect("commit");
    assert_eq!(result, 42);

    let err = pool.with_transaction(async |tx| {
        tx.execute(&format!("INSERT INTO {t} VALUES ($1)", t = t), &[&2i32]).await?;
        Err::<(), _>(XError::invalid("业务失败"))
    }).await.expect_err("rollback");
    assert_eq!(err.kind(), ErrorKind::Invalid);

    let n: i64 = pool.query_one(&format!("SELECT COUNT(*) FROM {t}"), &[]).await.expect("count").get("count");
...
async fn repo_crud_and_contracts() {
    ...
    let found = Repository::<PgRecord, String>::find(&repo, "int-repo-1".into()).await.expect("find");
    assert_eq!(found, Some(PgRecord { id: "int-repo-1".into(), data: b"data".to_vec() }));

    // upsert
    Repository::<PgRecord, String>::save(&repo, &PgRecord { id: "int-repo-1".into(), data: b"v2".to_vec() }).await.expect("upsert");
    let found2 = Repository::<PgRecord, String>::find(&repo, "int-repo-1".into()).await.expect("find");
    assert_eq!(found2.unwrap().data, b"v2");

    // missing
    assert_eq!(Repository::<PgRecord, String>::find(&repo, "nonexistent".into()).await.expect("find"), None);
...
async fn tx_runner_lifecycle() {
    ...
    let result = run_tx_lifecycle(&runner, || async { Ok::<_, XError>(42u32) }).await.expect("commit");
    assert_eq!(result, 42);
...
async fn migrator_verify_and_apply() {
    ...
    let m = Migration::new(1, "m1", format!("CREATE TABLE IF NOT EXISTS {p}_t (id INT)")).expect("m");
    let mut mig = Migrator::new(pool.clone(), vec![m]).expect("migrator");
...
async fn error_undefined_table_maps_to_missing() {
    let pool = PostgresPool::connect(&config()).await.expect("connect");
    let err = pool.execute("SELECT 1 FROM nonexistent_table_xyz", &[]).await.expect_err("missing table");
...
async fn pool_acquire_with_deadline() {
    ...
    assert!(zero.is_err());
    match zero {
        Err(e) => assert!(matches!(e.kind(), ErrorKind::Invalid | ErrorKind::DeadlineExceeded)),
        _ => {}
    }
    drop(pool);
}
...
fn config_defaults_and_builder() {
    ...
    let cfg = PostgresConfig::builder()
        .host("127.0.0.1").database("test").user("test")
        .sslmode(SslMode::Disable).max_pool_size(4)
        .build().expect("valid");
...
fn config_rejects_remote_plaintext() {
    let err = PostgresConfig::builder()
        .host("db.example.com").database("x").user("x")
        .build().expect_err("remote without ssl");
    assert_eq!(err.kind(), ErrorKind::Invalid);
}
...
fn migration_duplicate_version_rejected() {
    ...
    let pool_cfg = PostgresConfig::builder()
        .host("127.0.0.1").port(1).database("x").user("x")
        .sslmode(SslMode::Disable).acquire_timeout(Duration::from_millis(50))
        .max_pool_size(1).build().unwrap();
