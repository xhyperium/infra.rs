...
| `FOUNDATIONX_POSTGRESX_TLS_CLIENT_CERT` | mTLS 客户端证书 PEM | 与 KEY 成对 |
| `FOUNDATIONX_POSTGRESX_TLS_CLIENT_KEY` | mTLS 客户端私钥 PEM | 与 CERT 成对 |
...
- mTLS：提供 CERT+KEY 成对 PEM；仍强制校验服务端证书（无 insecure）
