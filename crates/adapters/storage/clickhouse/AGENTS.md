# clickhousex — Agent 行为规则

> 适用 crate：`crates/adapters/storage/clickhouse/`（包名 `clickhousex`）  
> 父级规则：[`crates/AGENTS.md`](../../../AGENTS.md)  
> SSOT 镜像：[`.agents/ssot/adapters/storage/clickhouse`](../../../../.agents/ssot/adapters/storage/clickhouse/)  
...
clickhouse storage adapter（当前为 scaffold）。
