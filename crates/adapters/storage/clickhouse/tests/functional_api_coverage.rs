...
/// 设置 ClickHouse 连接所需环境变量。
fn set_clickhouse_env() {
    set_env("FOUNDATIONX_CLICKHOUSEX_HOST", "127.0.0.1");
    set_env("FOUNDATIONX_CLICKHOUSEX_HTTP_PORT", "8123");
    set_env("FOUNDATIONX_CLICKHOUSEX_USER", "default");
    set_env("FOUNDATIONX_CLICKHOUSEX_PASSWORD", "iCEOuptIx40EduvGOKX73rfY");
}
...
fn config_from_env_reads_all_variables() {
    ...
    set_clickhouse_env();
    set_env("FOUNDATIONX_CLICKHOUSEX_DATABASE", "test_db");
    set_env("FOUNDATIONX_CLICKHOUSEX_TIMEOUT_MS", "30000");
    set_env("FOUNDATIONX_CLICKHOUSEX_MAX_IDLE_PER_HOST", "16");
    set_env("FOUNDATIONX_CLICKHOUSEX_MAX_IN_FLIGHT", "32");
    set_env("FOUNDATIONX_CLICKHOUSEX_ACQUIRE_TIMEOUT_MS", "10000");
...
fn config_from_env_tls_parsing() {
    ...
    for (val, expected) in &[
        ...
        set_env("FOUNDATIONX_CLICKHOUSEX_TLS", val);
        let c = ClickHouseConfig::from_env().expect("from_env");
...
fn config_port_alias_http_port_priority() {
    // HTTP_PORT 优先，PORT 兼容
    let result = resolve_http_port_test(Some("9440"), None, 8123);
    assert_eq!(result, 9440);

    // 仅 PORT 兼容
    let result = resolve_http_port_test(None, Some("8443"), 8123);
    assert_eq!(result, 8443);

    // 双设相同值 → Ok
    let result = resolve_http_port_test(Some("9440"), Some("9440"), 8123);
    assert_eq!(result, 9440);

    // 双设不同值 → 冲突拒绝
    let err = resolve_http_port_err(Some("8123"), Some("8443"), 8123);
    assert_eq!(err.kind(), ErrorKind::Invalid);
    assert!(err.to_string().contains("冲突"));
}

/// 通过环境变量测试端口别名。
fn resolve_http_port_test(http_port: Option<&str>, port_alias: Option<&str>, default: u16) -> u16 {
    clear_clickhouse_env();
    set_env("FOUNDATIONX_CLICKHOUSEX_HOST", "127.0.0.1");
    if let Some(v) = http_port {
        set_env("FOUNDATIONX_CLICKHOUSEX_HTTP_PORT", v);
    }
    if let Some(v) = port_alias {
        set_env("FOUNDATIONX_CLICKHOUSEX_PORT", v);
    }
    let c = ClickHouseConfig::from_env().unwrap_or_else(|e| {
        panic!("from_env 失败: {e}");
    });
    assert_eq!(c.http_port, default); // none 时用 default 或实际值
    // 直接从 resolve 逻辑验证
    if http_port.is_none() && port_alias.is_none() {
        return default;
    }
    // 重建验证：清理后只设对应的
    clear_clickhouse_env();
    ...
    if let Some(v) = http_port {
        set_env("FOUNDATIONX_CLICKHOUSEX_HTTP_PORT", v);
    } else if let Some(v) = port_alias {
        set_env("FOUNDATIONX_CLICKHOUSEX_PORT", v);
    }
    ClickHouseConfig::from_env().expect("from_env").http_port
}

fn resolve_http_port_err(
...
async fn pool_connect_from_env() {
    clear_clickhouse_env();
    set_clickhouse_env();
    set_env("FOUNDATIONX_CLICKHOUSEX_DATABASE", "default");
...
async fn pool_ensure_analytics_table_idempotent() {
    let pool = func_pool().await;
    drop_tbl(&pool, ANALYTICS_TABLE).await;
...
async fn analytics_sink_writes_and_verifies() {
    ...
    assert_eq!(rows[0][1], "payload-data");
    assert_eq!(rows[1][1], "payload-2");
