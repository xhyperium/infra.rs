...
//!
//! ```text
//! CLICKHOUSE_BENCH_PASSWORD=... cargo bench -p clickhousex --bench full_benchmark
//! ```
use std::fs;
...
async fn bench_insert_json_each_row(pool: &ClickHousePool) {
    ...
    for &batch in &[1usize, 10, 100, 1000] {
        ...
        report(&format!("clickhousex_insert_each_row_batch{batch}"), n, start.elapsed());
...
async fn bench_insert_batch(pool: &ClickHousePool) {
    ...
    for &chunk in &[100, 500, 1000, 5000] {
        ...
        report(&format!("clickhousex_insert_batch_chunk{chunk}"), n, start.elapsed());
        pool.execute(&format!("DROP TABLE IF EXISTS {table}")).await.expect("清理");
    }
}
...
async fn main() {
    ...
    let pool = match tokio::time::timeout(Duration::from_secs(5), ClickHousePool::connect(cfg.clone())).await {
        Ok(Ok(p)) => p,
        Ok(Err(error)) => { eprintln!("连接失败: {error}"); return; }
        Err(_) => { eprintln!("连接超时"); return; }
    };
    println!("\n# SELECT 1 延迟");
